...
local not_gl_error_msg = "You must be Guild Leader to do that";
	...
	local function OnClick_ImportLootPrio()
		...
		while txt_rem ~= nil do
			...
			local spec_class_max_count = #SKC.SPEC_MAP
			while prios_txt ~= nil do
				spec_class_cnt = spec_class_cnt + 1;
				valid = spec_class_cnt <= spec_class_max_count;
				if not valid then
					SKC:Error("Too many Class/Spec combinations. Expected "..spec_class_max_count);
					break;
				end
			...
			if (spec_class_cnt ~= spec_class_max_count) then
				valid = false;
				SKC:Error("Wrong number of Class/Spec combinations. Expected "..spec_class_max_count..", but got "..spec_class_cnt);
				break;
			end
